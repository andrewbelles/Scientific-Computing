...
#include "../Benchmark-Tool/benchmark.hpp"
...
int main(void)
{
  ...
  Hypergraph graph = Hypergraph("SI-Simple Graph");
  // Produces 120 of X 
  ...
  std::vector<PathStep> simple_path = graph.search_simple();
  print_graph(graph, simple_path, "Unoptimal Solution");

  // Multiple Input Graph 
  Hypergraph migraph = Hypergraph("MI-Intermediate Graph");
  ...
  std::vector<PathStep> misimple_path = migraph.search_simple();
  print_graph(migraph, misimple_path, "Unoptimal Solution");

  std::vector<PathStep> optimal_graph = migraph.search_optimal();
  print_graph(migraph, optimal_graph, "Optimal Solution");

  // Hypergraph cannot solve these problems 

  OptimizedHypergraph optimized_rotor = OptimizedHypergraph("OptimizedHypergraph - Rotor Factory");
  ...
  std::vector<PathStep> optimized_simple = optimized_rotor.search_simple();
  print_graph(optimized_rotor, optimized_simple, "Simple Solution");

  std::vector<PathStep> optimized_optimal = optimized_rotor.search_optimal();
  print_graph(optimized_rotor, optimized_optimal, "Optimal Solution");
  
  // OptimizedHypergraph fails to even compute the simple path 
